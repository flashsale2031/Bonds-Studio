...
type DefinitionMode = "2D" | "3D" | "Reality";
const definitionOptions: Array<{ id: DefinitionMode; label: string; description: string; frame: string }> = [
  { id: "2D", label: "2D", description: "Render and frame flat 2D scenes from this module.", frame: "2D scene frame" },
  { id: "3D", label: "3D", description: "Render and frame spatial 3D scenes on the Earth module.", frame: "3D spatial frame" },
  { id: "Reality", label: "Reality", description: "Frame surveillance footage inside the Earth module.", frame: "Reality surveillance frame" },
];
...
export default function ContactEarthGlobe({ contacts }: { contacts: ContactForEarth[] }) {
  ...
  const [locationQuery, setLocationQuery] = useState(""), [locationCommandDirty, setLocationCommandDirty] = useState(false), [selectedTime, setSelectedTime] = useState("LIVE"), [appliedTime, setAppliedTime] = useState("LIVE"), [definitionMode, setDefinitionMode] = useState<DefinitionMode>("3D"), [commandLocation, setCommandLocation] = useState<WorldLocation | null>(null), [commandMessage, setCommandMessage] = useState("");
  ...
  function submitEarthCommand(event: FormEvent<HTMLFormElement>) {
    ...
    setSelected(null);
    const definition = definitionOptions.find(option => option.id === definitionMode) || definitionOptions[1];
    setCommandMessage(`Centered on ${candidate.name}, ${countryName(candidate.countryCode)} at ${selectedTime === "LIVE" ? "live orbital time" : selectedTime} · ${definition.frame}.`);
  }

  function changeDefinitionMode(value: string) {
    const next = value as DefinitionMode;
    const definition = definitionOptions.find(option => option.id === next) || definitionOptions[1];
    setDefinitionMode(next);
    setCommandMessage(`${definition.label} definition selected · ${definition.description}`);
  }

  const activeDefinition = definitionOptions.find(option => option.id === definitionMode) || definitionOptions[1];

  return <section className={`contact-earth-module definition-${definitionMode.toLowerCase()} border border-[#17231e]/10 bg-[#f9f5ec] overflow-hidden`}>
    <div className="contact-earth-header border-b border-[#17231e]/10 bg-[#1a2820] p-5 text-white">
      <div className="contact-earth-header-row flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        ...
        <button onClick={() => { setPins([]); setFailed([]); }} className="contact-earth-refresh inline-flex items-center justify-center gap-2 border border-white/15 px-3 py-2 text-xs"><RefreshCw className="h-3.5 w-3.5" />Refresh</button>
      </div>
      <form className="contact-earth-command-menu mt-5 grid gap-3 border border-white/10 bg-black/10 p-3 md:grid-cols-[110px_minmax(220px,1fr)_180px_auto] md:items-end" onSubmit={submitEarthCommand} aria-label="Earth definition, location, and time command">
        <label className="contact-earth-definition-field font-ledger text-[9px] uppercase tracking-[0.18em] text-[#c9a84c]" htmlFor="contact-earth-definition"><span>Definition</span><select id="contact-earth-definition" value={definitionMode} onChange={event => changeDefinitionMode(event.target.value)} className="contact-earth-definition-select mt-1 block w-full border border-white/15 bg-[#071b26]/65 px-3 py-2.5 text-xs text-white outline-none" aria-label="Scene definition">{definitionOptions.map(option => <option key={option.id} value={option.id}>{option.label}</option>)}</select></label>
        <div className="contact-earth-location-field relative"><label htmlFor="contact-earth-location" className="font-ledger text-[9px] uppercase tracking-[0.18em] text-[#c9a84c]">Location</label><div className="contact-earth-input-row mt-1 flex items-center gap-2 border border-white/15 bg-[#071b26]/65 px-3"><Search className="h-3.5 w-3.5 shrink-0 text-[#c9a84c]" /><input id="contact-earth-location" value={locationQuery} onChange={event => { setLocationQuery(event.target.value); setLocationCommandDirty(true); setCommandMessage(""); }} placeholder="City or populated place" autoComplete="off" aria-autocomplete="list" aria-expanded={locationCommandDirty && suggestions.length > 0} aria-controls="contact-earth-location-suggestions" className="contact-earth-location-input min-w-0 flex-1 bg-transparent py-2.5 text-xs text-white outline-none placeholder:text-white/40" /><span className="hidden font-ledger text-[8px] uppercase tracking-widest text-white/35 md:inline">world array</span></div>{locationCommandDirty && suggestions.length > 0 && <div id="contact-earth-location-suggestions" role="listbox" className="contact-earth-suggestions absolute left-0 right-0 top-full z-30 mt-2 max-h-64 overflow-auto border border-[#c9a84c]/50 bg-[#10251f] p-1 shadow-2xl">{suggestions.map(location => <button type="button" key={location.id} role="option" onClick={() => { setLocationQuery(`${location.name}, ${countryName(location.countryCode)}`); setLocationCommandDirty(true); }} className="grid w-full gap-1 border-b border-white/10 px-3 py-2 text-left last:border-0 hover:bg-white/10"><span className="text-xs font-semibold text-white">{location.name}</span><small className="font-ledger text-[8px] uppercase tracking-wider text-white/55">{countryName(location.countryCode)} · {coordinate(location.lat, "N", "S")} · {coordinate(location.lng, "E", "W")}</small></button>)}</div>}</div>
        <label htmlFor="contact-earth-time" className="contact-earth-time-field font-ledger text-[9px] uppercase tracking-[0.18em] text-[#c9a84c]">Time<select id="contact-earth-time" value={selectedTime} onChange={event => setSelectedTime(event.target.value)} className="contact-earth-time-select mt-1 block w-full border border-white/15 bg-[#071b26]/65 px-3 py-2.5 text-xs text-white outline-none">{timeOptions.map(time => <option key={time}>{time}</option>)}</select></label>
        <button type="submit" className="contact-earth-submit inline-flex min-h-[39px] items-center justify-center gap-2 bg-[#c9a84c] px-5 py-2.5 text-xs font-bold uppercase tracking-wider text-[#17231e] hover:bg-[#e0c66c]"><Send className="h-3.5 w-3.5" />Enter</button>
      </form>
      <div className="contact-earth-command-status mt-2 flex flex-wrap items-center justify-between gap-2 font-ledger text-[8px] uppercase tracking-wider text-white/50"><span>{commandMessage || (commandLocation ? `Centered scene · ${commandLocation.name}, ${countryName(commandLocation.countryCode)}` : activeDefinition.description)}</span><span>{commandLocation ? `${activeDefinition.frame} · ${appliedTime === "LIVE" ? "LIVE" : appliedTime} · ${coordinate(commandLocation.lat, "N", "S")} · ${coordinate(commandLocation.lng, "E", "W")}` : `${activeDefinition.frame} · Source refresh · ${WORLD_LOCATIONS_DATA_UPDATED}`}</span></div>
    </div>
    <div className="contact-earth-body grid gap-6 p-5 lg:grid-cols-[minmax(0,1fr)_280px]">
      <div className={`contact-earth-globe relative mx-auto aspect-square w-full max-w-[620px] overflow-hidden rounded-full bg-[#071b26] shadow-[inset_0_0_70px_rgba(0,0,0,.8),0_20px_55px_rgba(26,40,32,.18)] touch-none ${commandLocation ? "is-centered" : ""}`} onPointerMove={e => { if (e.buttons === 1) setCenter(value => ({ lon: value.lon - e.movementX * .35, lat: Math.max(-75, Math.min(75, value.lat + e.movementY * .25)) })); }} onWheel={e => { e.preventDefault(); setCenter(value => ({ ...value, lon: value.lon + e.deltaY * .12 })); }}>
